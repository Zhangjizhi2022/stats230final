...
In the Informed Sub-Sampling method, the set of good subsamples is treated as a series of missing data (denoted by $U_{1}$, $U_{2}$, and so on) and is simulated using the ISS-MCMC algorithm. This algorithm generates a Markov chain on the extended space $\theta \times  \textsf{U}_{n}$. The sequence of sub samples is randomly updated in a way that favors those subsets with summary statistics that are similar to the full dataset, as suggested by the analysis in previous sections. The process is implemented using a symmetric transition kernel $R$ on $(\textsf{U}_n, \mathcal{U}_n)$. A transition from $(\theta_i,U_i)$ to $(\theta_{i+1} , U_{i+1})$ involves two steps:
...
Here we implement one example from their paper. Their example involves simulated logistic regression data with $N= 10^6$ observations. However given our limited computational memory, we could not implement the full $N = 10^{6}$ simulated example. Instead, we ran our simulation for $N = 10^5$ using and $n = 100, 500$ and $1000$ and compared our results to the MRH without sub sampling.
...
For the summary statistic $S$ is the sum of $Y_{U}$ since our data is binomial. The transition kernel of subsets $R(U_i,U)$ is defined as a random sample from the set $\textsf{U}_{n}$. The transition kernel of the parameter $Q(\beta_i,\beta)$ is defined as a uniform distribution in $\mathbb{R}^3$ with mean $\beta_i$ and width $\delta=0.2$. We chose this proposal density because it simplifies the calculation of the acceptance ratio in the ISS-MCMC algorithm.

Finally, in implementing the logistic regression example, we used two different tuning parameter values for $\epsilon$;  $\epsilon=0.00005$ and $\epsilon=1$. The authors of the paper are not clear on how they chose their $\epsilon$ values. See section 6.3 of @Maire2019. In some parts of the paper it seems like $\epsilon$ is $5\times 10^{4}$, while in other parts it appears that $\epsilon$ is $5 \times 10^{-6}$. So we made this choice because for large values of $\epsilon > 1$ we did not see any change in the acceptance rate of a new proposal $U_{i + 1}$.
...
```{r "new computing time", echo=FALSE}
mrhtime <-system.time({MRH_MCMC_logistic(sim,y, d, delta)})
iss_100 <-system.time({ISS_MCMC_logistic(sim, y, n=100, d, delta, epsilon, iter)})
iss_500 <-system.time({ISS_MCMC_logistic(sim, y, n=500, d, delta, epsilon, iter)})
iss_1000<-system.time({ISS_MCMC_logistic(sim, y, n=1000, d, delta, epsilon, iter)})
...
colnames(comparison) <- c("System Time (s)")
...
```{r "saving output", echo=FALSE, cache=TRUE}
...
#trace plot
plot(r4[1,],main=TeX(r'(trace plot of $\beta_{0}$ in ISS_MCMC method for n=1000)'),
     type='l', xlab="Iteration",
     ylab=TeX(r'($\beta_{0}$)'))
plot(r4[2,],main=TeX(r'(trace plot of $\beta_{1}$ in ISS_MCMC method for n=1000)'),
     type='l', xlab="Iteration",
     ylab=TeX(r'($\beta_{1}$)'))
plot(r4[3,],main=TeX(r'(trace plot of $\beta_{2}$ in ISS_MCMC method for n=1000)'),
     type='l', xlab="Iteration",
     ylab=TeX(r'($\beta_{2}$)'))

# for n=100
#beta0
plot(density(r1[1,]),xlab =TeX(r'($\beta_{0}$)'),ylab = 'density',xlim=c(-1.5,-0.5),main=TeX(r'(Density plots for $\beta_{0}$ when n=100)'))
...
#beta1
plot(density(r1[2,]),xlab = TeX(r'($\beta_{1}$)'),ylab = 'density',xlim=c(0.5,1.5),main=TeX(r'(Density plots for $\beta_{1}$ when n=100)'))
...
#beta2
plot(density(r1[3,]),xlab =TeX(r'($\beta_{2}$)'),ylab = 'density',xlim=c(-0.5,0.5),main=TeX(r'(Density plots for $\beta_{2}$ when n=100)'))
lines(density(r2[3,]),col='red')
lines(density(r5[3,]),col='blue')
legend("topright", legend = c("MH", "ISS epsilon=0.00005", "ISS epsilon=0"), 
       col = c("black", "red", "blue"), lwd = 2)

# for n=500
#beta0
plot(density(r1[1,]),xlab =TeX(r'($\beta_{0}$)'),ylab = 'density',xlim=c(-1.5,-0.5),main=TeX(r'(Density plots for $\beta_{0}$ when n=500)'))
...
#beta1
plot(density(r1[2,]),xlab = TeX(r'($\beta_{1}$)'),ylab = 'density',xlim=c(0.5,1.5),main=TeX(r'(Density plots for $\beta_{1}$ when n=500)'))
...
#beta2
plot(density(r1[3,]),xlab =TeX(r'($\beta_{2}$)'),ylab = 'density',xlim=c(-0.5,0.5),main=TeX(r'(Density plots for $\beta_{2}$ when n=500)'))
lines(density(r3[3,]),col='red')
lines(density(r6[3,]),col='blue')
legend("topright", legend = c("MH", "ISS epsilon=0.00005", "ISS epsilon=0"), 
       col = c("black", "red", "blue"), lwd = 2)
...
# for n=1000
#beta0
plot(density(r1[1,]),xlab =TeX(r'($\beta_{0}$)'),ylab = 'density',xlim=c(-1.5,-0.5),main=TeX(r'(Density plots for $\beta_{0}$ when n=1000)'))
...
#beta1
plot(density(r1[2,]),xlab = TeX(r'($\beta_{1}$)'),ylab = 'density',xlim=c(0.5,1.5),main=TeX(r'(Density plots for $\beta_{1}$ when n=1000)'))
...
#beta2
plot(density(r1[3,]),xlab =TeX(r'($\beta_{2}$)'),ylab = 'density',xlim=c(-0.5,0.5),main=TeX(r'(Density plots for $\beta_{2}$ when n=1000)'))
lines(density(r4[3,]),col='red')
lines(density(r7[3,]),col='blue')
legend("topright", legend = c("MH", "ISS epsilon=0.00005", "ISS epsilon=0"), 
       col = c("black", "red", "blue"), lwd = 2)
```
